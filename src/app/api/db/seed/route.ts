...
export async function GET() {
  try {
    ...
    // 2. Create User Profiles (Admin & Residents)
    ...
    // Tenants (20 total: 16 active, 4 left)
    const tenants = [
    ...
    // Seed users collection so tenants can login
    ...
    // Payments logs (detailed for dynamic dues and payment histories)
    const payments = [
      // June 2026 Paid
      ...
      // Inactive Left Tenants' final payments (all paid)
      ...
      // Historical logs (for rich payment history)
    ...
    await db.collection('maintenance').insertMany(maintenance);

    return NextResponse.json({
      success: true,
      message: 'Database seeded successfully with 1 Admin user, 12 Rooms, 20 Tenants, and payment ledger entries!'
    });
  } catch (err: any) {
